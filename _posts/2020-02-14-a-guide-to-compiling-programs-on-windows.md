...
can get by running `dir /b "%VCInstallDir%Tools\MSVC"`. You might need to
...
| Name     | Description                                          |
|----------|------------------------------------------------------|
| `ucrt`   | Univeral C Runtime Library                           |
...
no-op. `UNICODE` transfroms all Windows API functions (in this case
...
10, there's a new beta option that extends the ANSI fuctions with UTF-8
...
Now we're finally read to use the library in our program. Instead of calling
